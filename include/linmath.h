...
LINMATH_H_FUNC void mat4x4_zero(mat4x4 M)
{
	int i, j;
	for(i=0; i<4; ++i)
		for(j=0; j<4; ++j)
...
LINMATH_H_FUNC void mat4x4_mul(mat4x4 M, mat4x4 const a, mat4x4 const b)  // b 左乘 a: M = b * a
{
	mat4x4 temp;
	int k, r, c;
...
LINMATH_H_FUNC void mat4x4_mul_vec4(vec4 r, mat4x4 const M, vec4 const v) //  r = v * M
{
	int i, j;
	for(j=0; j<4; ++j) {
		r[j] = 0.f;
...
LINMATH_H_FUNC void mat4x4_left_mul_vec4_in_col(vec4 r, mat4x4 const M, vec4 const v) //  r =  M * v
{
	int i, j;
	for(j=0; j<4; ++j) {
		r[j] = 0.f;
...
LINMATH_H_FUNC void mat4x4_perspective(mat4x4 m, float y_fov, float aspect, float n, float f)
{
	...
	float const a = 1.f / tanf(y_fov / 2.f); // a = fy / cy
...
LINMATH_H_FUNC void mat4x4_perspective_from_intrinsic(mat4x4 m, float fx, float fy, float cx, float cy, float near, float far, float imgW, float imgH)
{
	m[0][0] = 2.0f * fx / imgW;
	m[0][1] = 0.0f;
	m[0][2] = 0.0f;
	...
	m[2][0] = 1.0f - (2.0f * cx / imgW);
	m[2][1] = (2.0f * cy / imgH) - 1.0f;
...
LINMATH_H_FUNC void mat4x4_perspective_wan(mat4x4 m, float x_fov, float y_fov, float n, float f)
{
	/* NOTE: Degrees are an unhandy unit to work with.
	 * linmath.h uses radians for everything! */

	m[0][0] = 1.f / tanf(x_fov / 2.f);
	m[0][1] = 0.f;
	m[0][2] = 0.f;
	m[0][3] = 0.f;

	m[1][0] = 0.f;
	m[1][1] = 1.f / tanf(y_fov / 2.f);
	m[1][2] = 0.f;
	m[1][3] = 0.f;
...
LINMATH_H_FUNC void mat4x4_from_quat(mat4x4 M, quat const q)  // left hand JPL convertion
{
	float a = q[3]; // w
	float b = q[0]; // x
	float c = q[1]; // y
...
LINMATH_H_FUNC void mat4x4_from_quat_right_hand(mat4x4 M, quat const q)
{
	float a = q[3]; // w
	float b = q[0]; // x
	float c = q[1]; // y
